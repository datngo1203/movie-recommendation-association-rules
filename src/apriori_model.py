...
OUTPUT_FILE = PROJECT_ROOT / "models" / "apriori_rules.csv"
RULE_COLUMNS = [
    "algorithm",
    "mining_method",
    "transactions",
...
def build_apriori_rules(
    ...
    for transaction in transactions:
        ...
        pair_counts.update(combinations(filtered_items, 2))
    ...
    for (left, right), pair_count in pair_counts.items():
        ...
        for antecedent, consequent in ((left, right), (right, left)):
            ...
            rules.append(
                {
                    "algorithm": "Apriori",
                    "mining_method": "candidate_generation",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Apriori association rules for movie recommendations.")
    ...
    print(f"Apriori: da luu {len(rules)} rules vao {args.output} ({elapsed_seconds}s)")
